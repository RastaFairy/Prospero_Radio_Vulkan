...
class Ps5VulkanRenderInterface final : public Rml::RenderInterfaceCompatibility
{
  ...
  private:
    ...
    bool CreateImage(std::uint32_t width, std::uint32_t height, VkFormat format, VkImageUsageFlags usage,
                     VkImage &image, VkDeviceMemory &memory, std::uint32_t mip_levels = 1);
    bool CreateTextureFromPixels(const std::uint8_t *pixels, std::uint32_t width, std::uint32_t height,
    ...
    bool ReadTextureFile(const Rml::String &source, std::vector<std::uint8_t> &pixels, int &width, int &height);
    bool DecodeRadioAtlas(const std::vector<std::uint8_t> &payload, std::vector<std::uint8_t> &pixels,
                          std::uint32_t width, std::uint32_t height);
    std::vector<std::uint8_t> ReadBinaryFile(const char *path);
    bool ReadTextTextureHeader(const std::vector<std::uint8_t> &bytes, int &width, int &height) const;

...
## FIX20 — PS5_Vulkan checkout reset and stale warning-policy guard

La ejecución real de FIX19 siguió abortando en el helper de warning policy aunque la forma del `make` ya era correcta. El problema restante es de estado del checkout cacheado de PS5_Vulkan.

### Causa

`overlay/setup-ps5-vulkan.sh` hacía:

- `git fetch --depth 1 origin "$PS5_VULKAN_REF"`
- `git checkout --detach "$PS5_VULKAN_REF"`

pero no ejecutaba `git reset --hard`. Si `tools/build-driver.sh` o `tools/build-psbc-ps5.sh` habían sido modificados por una ejecución anterior del overlay, Git podía conservar esos cambios locales aun con `HEAD` apuntando al commit correcto. La siguiente versión del patcher entonces buscaba el texto original y encontraba 0 coincidencias porque ya estaba presente una política anterior.

### Corrección

FIX20:

1. Añade `git reset --hard "$PS5_VULKAN_REF"` inmediatamente después del checkout.
2. Cambia `patch-ps5-vulkan-warning-policy.py` a `mesa-warning-policy-v5`.
3. Añade `guard_stale_warning_policy()` a las rutas PS5 y host-PIC. Si pese al reset aparece `mesa-warning-policy-vN` de una versión anterior, el helper termina con un diagnóstico explícito indicando que el checkout debe ser restaurado al commit.
4. La versión actual (`v5`) continúa siendo idempotente.
5. No se altera la política funcional: PSBC de terceros sigue con `-w` por defecto y `PS5VK_PSBC_WARNING_AUDIT=1` reabre los warnings para auditoría.

### Validación FIX20

- `python3 -m py_compile overlay/tools/patch-ps5-vulkan-warning-policy.py` — OK.
- `bash -n overlay/setup-ps5-vulkan.sh` — OK.
- Aplicación sobre scripts sintéticos con la forma exacta de los `make` fijados — OK.
- `bash -n` de los scripts resultantes — OK.
- Segunda aplicación del patcher — idempotente — OK.
- Inyección de `mesa-warning-policy-v4` — el guard devuelve error explícito: `stale warning policy detected (mesa-warning-policy-v4); PS5_Vulkan must be reset to commit before patching`.

### Estado

FIX20 corrige la persistencia de modificaciones locales del checkout PS5_Vulkan entre ejecuciones. Todavía no se declara éxito del build real de PSBC ni del enlace final hasta disponer de una ejecución WSL2 posterior a FIX20.


## FIX21 — Mesa NIR intrinsic table required by final title link

La compilación real posterior a FIX20 avanzó hasta el enlace/conversión del título. PSBC compiló correctamente con **0 warnings**, el driver PS5 compiló correctamente y el audit de duplicados no encontró símbolos fuertes duplicados. El bloqueo final fue:

`error: no public SDK stub exports required symbol nir_intrinsic_infos`

La causa inmediata es que el linker del título permite indefinidos para imports de módulos SCE y el `ps5-native-tool` detecta posteriormente que `nir_intrinsic_infos` sigue sin resolverse. `nir_intrinsic_infos` no es un import de PS5: es la tabla global generada por Mesa NIR en `src/compiler/nir/nir_intrinsics.c`. El Makefile upstream define ese archivo como fuente generada de NIR.

FIX21 realiza tres controles/correcciones:

1. `overlay/setup-ps5-vulkan.sh` materializa explícitamente los generados NIR `nir_intrinsics.h`, `nir_intrinsics.c` y `nir_intrinsics_indices.h` antes de ensamblar el archive PS5.
2. `overlay/apply-vulkan.py`: cuando hay archivos Vulkan estáticos, añade `--undefined=nir_intrinsic_infos` al `link_inputs` antes de los archivos estáticos, forzando a LLD a extraer la definición desde el archivo PSBC correspondiente en vez de dejar el símbolo como indefinido dentro del modo de importación del título. Además añade una auditoría post-link específica para cualquier símbolo `nir_*` indefinido.
3. `overlay/setup-ps5-vulkan.sh`: después de compilar PSBC verifica que `libpsbc.ps5.a` realmente define `nir_intrinsic_infos`, y después del driver verifica que `libpsbc_driver.ps5.a` también lo define. Esto evita que una futura regresión de fuentes generadas quede oculta hasta el converter.

Se verificó localmente `py_compile` y `bash -n`.

Estado: pendiente de ejecución real de FIX21 en WSL2; no se declara todavía `eboot.bin`, FFPKG ni FFPFSC válidos.
